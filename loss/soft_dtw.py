...
def pairwise_distances_asymmetric(x, y=None, cu=0.5, co=2, window_size = 5, threshold=0.5):
    ...
    # Compute trend
    x_start = x[:-window_size].view(-1, 1)
    x_end = x[window_size:].view(-1, 1)
    target_diff = x_end - x_start

    pred_diff = y[-x_start.shape[0]:] - x_start
    trend_pos = torch.logical_and(torch.sign(target_diff) == torch.sign(pred_diff), (torch.abs(target_diff) > threshold))
    trend_neg = torch.logical_and(torch.sign(target_diff) != torch.sign(pred_diff), (torch.abs(target_diff) > threshold))

    dist[window_size:, :] = torch.where(trend_pos, dist[window_size:, :] * cu, dist[window_size:, :])
    dist[:-window_size, :] = torch.where(trend_neg, dist[:-window_size, :] * co, dist[:-window_size, :])
    return dist
